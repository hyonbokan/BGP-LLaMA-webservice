...
**Live demo (retired):** hosted at `llama.cnu.ac.kr` during the thesis (CNU provided the domain
for a ~6-month arrangement) and no longer online.
...
|  |  |
| :---: | :---: |
| ![Home page hero and looking-glass readout](docs/screenshots/home.png)<br>_Home: hero and looking-glass_ | ![BGP Chat workspace](docs/screenshots/chat.png)<br>_BGP Chat: model switch, streaming, script output_ |
| ![Datasets listing](docs/screenshots/datasets.png)<br>_Datasets: instruction corpus_ | ![Dataset detail page](docs/screenshots/detail.png)<br>_Dataset detail: sample record and topics_ |
...
[opencode](https://opencode.ai) on `PATH` (`opencode --version` → 1.17.18; or run the pod's
Docker image instead, see its `DEPLOY.md`), and a provider key for the agent model.

**1 · Start the pod** (Terminal A), holds the keys and the agent loop:
...
### How the workspace reaches the pod

`WORKSPACE_TRANSPORT` decides how the gathered BGP data gets to the pod:

- `file` (default): the backend passes a `file://` path. Simplest, but the pod must share this
  host's filesystem, so it only fits single-host dev.
- `minio`: the backend tars the workspace, uploads it to an S3-compatible store, and hands the pod
  a pre-signed `https://` URL it pulls over the network. The pod then shares no filesystem with the
  backend, which is the shape a real cross-host deploy needs. Configured with `MINIO_ENDPOINT`,
  `MINIO_ACCESS_KEY`, `MINIO_SECRET_KEY`, `MINIO_BUCKET`, `MINIO_SECURE`, `MINIO_CERT_CHECK`.

### Isolated deployment (`make up-agent`)

`docker-compose.pod.yml` runs the security-correct topology where the pod shares no filesystem or
network with this backend: the pod on an `internal` network with no route to the internet, MinIO as
neutral workspace storage over TLS, and a deny-by-default egress proxy that allowlists only the
provider hosts. The pod reaches OpenAI only through that proxy and pulls workspaces from MinIO over
the internal network.

```bash
./scripts/gen_minio_cert.sh        # once: self-signed dev cert for MinIO
# set AGENT_POD_TOKEN + a provider key in .env
make up-agent                      # api + nginx + pod + minio + egress (GPT path, no GPU)
```
...
- Workspace transport: `WORKSPACE_TRANSPORT` (`file`|`minio`), and for `minio`: `MINIO_ENDPOINT`,
  `MINIO_ACCESS_KEY`, `MINIO_SECRET_KEY`, `MINIO_BUCKET`, `MINIO_SECURE`, `MINIO_CERT_CHECK`,
  `MINIO_URL_EXPIRY_SECONDS`. See [How the workspace reaches the pod](#how-the-workspace-reaches-the-pod)
