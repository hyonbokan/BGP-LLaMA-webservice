...
// The 12-topic BGP taxonomy is identical across several datasets in the source;
// defined once here and shared to keep the data DRY.
const BGP_CATEGORIES: Record<string, string> = {
  '1. BGP Fundamentals':
    'Covers the foundational principles of BGP, including how BGP functions as a path vector protocol and the basic mechanisms behind BGP route advertisements, decision processes, and convergence.',
  '2. BGP Security and Stability':
    'Focuses on the security challenges within BGP, such as prefix hijacking, route leaks, and methods to secure BGP routing, including RPKI (Resource Public Key Infrastructure) and filtering mechanisms.',
  '3. BGP Route Stability':
    'Explores the stability of BGP routes, path flapping, and the effect of route oscillation on global internet routing, including mitigation techniques such as route dampening.',
  '4. BGP Route Manipulation and Optimization':
    'Covers topics related to BGP route manipulation through policies and attributes such as local preference, MED, and AS-path prepending to optimize traffic flow and network performance.',
  '5. BGP Analysis and Anomaly Detection':
    'Focuses on detecting anomalies in BGP routing, such as route hijacks, leaks, and malicious activities, using tools and methods to analyze routing tables and BGP announcements.',
  '6. BGP Session Establishment and Maintenance':
    'Details the processes involved in establishing and maintaining BGP sessions between peers, including TCP connection setup, keepalive messages, and session termination.',
  '7. BGP Peer Grouping and Configuration':
    'Explains how BGP peer groups can be used to simplify configuration and enhance the scalability of routing policies, especially in large-scale networks.',
  '8. BGP Address Families':
    'Explores different BGP address families such as IPv4, IPv6, and VPNv4, and their respective roles in routing multiple protocol families within BGP.',
  '9. BGP Policies and Route Maps':
    'Covers the use of route maps, prefix lists, and filter policies to control the advertisement and acceptance of routes, and how these policies affect global routing decisions.',
  '10. BGP Multihoming and Load Balancing':
    'Examines how BGP is used for multihoming to improve redundancy and reliability in networks, including load balancing techniques across multiple ISPs.',
  '11. BGP Troubleshooting and Diagnostics':
    'Details tools and techniques for diagnosing and troubleshooting BGP issues, such as using BGP debugs, route tracing, and route validation to identify misconfigurations or network problems.',
  '12. BGP Case Studies and Real-World Examples':
    'Presents real-world BGP incidents and case studies, such as high-profile BGP hijacks or outages, and explains how these events impacted global routing and network stability.',
};

export const datasetSections: DatasetSection[] = [
  {
    ...
    datasets: [
      {
        id: 'bgp-knowledge',
        title: 'BGP Knowledge',
        fileUrl: 'self-instruct-generated-dataset/bgp-knowledge.json',
        fileCount: 20000,
        size: '18.1 MB',
        fileType: 'json',
        description:
  ...
  },
  {
    ...
    datasets: [
      ...
      },
      {
        ...
        fileUrl: '/manual-seed-dataset/bgp-knowledge-2.jsonl',
        promptUrl: '/manual-seed-dataset/prompt_knowledge.txt',
        fileCount: 36,
        size: '49 KB',
        fileType: 'jsonl',
      ...
      },
      {
        ...
        title: 'PyBGPStream Real-time Stream',
        fileUrl: '/manual-seed-dataset/bgp-pybgpstream-real-time.jsonl',
        promptUrl: '/manual-seed-dataset/prompt-pybgpstream-realtime.txt',
        fileCount: 4,
        size: '12 KB',
        fileType: 'jsonl',
        promptType: 'txt',
        description:
          'This dataset focuses on real-time BGP data analysis using PyBGPStream with the RIS Live collection. The scripts are designed to analyze specific prefix or ASN announcements, providing detailed insights into routing behavior and potential anomalies. By focusing on prefix and ASN-specific analysis, this dataset allows for a more granular understanding of routing changes, disruptions, and patterns within the BGP ecosystem. Such targeted analysis is crucial for identifying abnormal routing behavior, monitoring the stability of specific networks, and gaining insights into the propagation of route announcements across the internet. The dataset also includes visualization scripts to effectively illustrate the observed statistics.',
        jsonSnapshot: {
          id: 'seed_task_0',
          name: 'AS18298 Real-Time BGP Data Analysis',
          instruction:
            'Collect BGP update data for AS18298 over a specific collection period. Record prefix announcements and withdrawals during this period. Aggregate and visualize the collected data.',
          instances: [
            {
              input: 'Collection Period: 72000 seconds\nTarget AS: 18298',
              output:
                'import pybgpstream\nimport pandas as pd\nimport matplotlib.pyplot as plt\nimport time\n\ncollection_period = 72000\ntarget_as = "18298"\n\nstream = pybgpstream.BGPStream(\n    project="ris-live",\n    record_type="updates",\n)\n\n# ... collect announcements/withdrawals for target_as, then aggregate and plot.',
            },
          ],
          is_classification: false,
        },
...
export function findSection(sectionId?: string) {
  return datasetSections.find((s) => s.id === sectionId);
}

export function findDataset(sectionId?: string, datasetId?: string) {
  return findSection(sectionId)?.datasets.find((d) => d.id === datasetId);
}
